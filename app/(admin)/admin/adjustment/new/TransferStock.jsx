...
import { GetsingleWarehouse } from "../../../../../actions/WareHouse";
import { StockTransfer } from "../../../../../actions/Inventory";
...
const AdjustmentSchema = z.object({
  ...
  recievingWarehouse: z.string().min(2, {
    message: "Warehouse must be at least 2 characters.",
  }),
});

const TransferStock = ({ products, warehouse }) => {
  const [loading, setLoading] = useState(false);
  const [value, setValue] = useState("");
  const [recieveWarehouse, setRecievingWarehouse] = useState(null);
  ...
  console.log("recieveWarehouse", recieveWarehouse);

  const recieving = form.watch("recievingWarehouse");
  console.log("recievingWarehouse", recieving);
  useEffect(() => {
    async function getWarehouse() {
      if (recieving?.length === 0) return;
      try {
        const res = await GetsingleWarehouse(recieving);
        setRecievingWarehouse(res[0]);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await StockTransfer(data, value, recieveWarehouse);
      console.log(res);
      if (res?.ok) {
        setLoading(false);
              ...
              >
                <div className="flex flex-col gap-8  overflow-auto">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className={" w-full"}>
                          <FormLabel>Refrence No</FormLabel>
                          <FormControl>
                  ...
                  <div className="w-full flex flex-wrap  items-center gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className=" max-w-72 w-full">
                          <FormLabel>Transfering Warehouse</FormLabel>
                          <FormControl>
                    ...
                    <FormField
                      controle={form.control}
                      name="recievingWarehouse"
                      render={({ field }) => (
                        <FormItem className={"max-w-72 w-full"}>
                          <FormLabel>Recieving Warehouse</FormLabel>
                          <FormControl>
                            <Select onValueChange={field.onChange} {...field}>
                              <SelectTrigger className=" max-w-md w-full">
                                <SelectValue placeholder="Warehouse" />
